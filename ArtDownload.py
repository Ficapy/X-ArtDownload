...
def Gettitle(url):
    headers = { 'Use-Agent':'Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.8.1.14) Gecko/20080404 (FoxPlus) Firefox/2.0.0.14' }
    req = urllib2.Request(url,headers = headers)
    page = urllib2.urlopen(req).read()
    #<img src="http://x-art.com/videos/pink_on_the_inside/thumb_2.jpg" alt="Pink on the Inside">
    title = re.findall(r'<img src="http://x-art.com/videos/.+.jpg" alt="(.+?)">',page)
    return title


def Getmagnet(single):
    assert type(single) == type('')
    single = single.lower()
    headers = { 'Accept-Encoding':	'gzip, deflate',
    'Use-Agent':'Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.8.1.14) Gecko/20080404 (FoxPlus) Firefox/2.0.0.14'
    }
    #搜索页面
    searchlink = r'http://kickass.to/usearch/'+urllib2.quote(single+' '+'art')
    searchreq = urllib2.Request(searchlink,headers = headers)
    html_search = urllib2.urlopen(searchlink).read()
    html_search = gzip.GzipFile(fileobj=StringIO.StringIO(html_search), mode="r").read()
    if re.search(r'<span>  results .+ from \d+</span>',html_search):
        sourcelink = re.search(r'<a rel=".+" class=".+" href="(/.+?\.html)#comment">',html_search).group(1).decode('UTF-8')
